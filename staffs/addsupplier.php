...
if (isset($_POST['staffsub'])) {
	...
	$supp=$_POST['suppname'];
	$supcon=$_POST['supcon'];
	$suplocation=$_POST['suploc'];
	$supsource=$_POST['supsour'];
	$supstatus=$_POST['supstatus'];

	$check="SELECT * FROM suppliers WHERE Bname='$supb' AND Pname='$supp'";
	$run=mysqli_query($connect,$check);
	$count=mysqli_num_rows($run);

	if ($count>0) {

		echo"<script>window.alert('Supplier Name already exist')</script>";
		  header("Refresh:0");
		
	}
	else{
		$insert=" INSERT INTO suppliers(`SupplierID`, `Bname`, `Pname`, `Contact`, `Location`, `Source`,`supstatus`) VALUES ('$supcode','$supb','$supp','$supcon','$suplocation','$supsource','$supstatus')" ;
...
<li>
<a href="addsupplier.php" class="active"> Add Suppliers</a>
</li>
...
<input type="text" name="supbname" required>
</div>
</div>
...
<div class="form-group">
<label>Contact</label>
<input type="number" name="supcon" id="phoneNumberInput" onchange="validatePhoneNumber()" required>
</div>
</div>
<div class="col-lg-4 col-sm-6 col-12">
<div class="form-group">
<label>Location</label>
<input type="text" name="suploc" required>
</div>
</div>
<div class="col-lg-4 col-sm-6 col-12">
<div class="form-group">
<label>Supply Source</label>
<input type="text" name="supsour" required>
</div>
</div>
...
<div class="form-group">
			...
			<input type="radio" name="supstatus" value="Active" checked>Active
