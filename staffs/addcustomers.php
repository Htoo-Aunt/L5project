...
if (isset($_POST['staffsub'])) {
	...
	$supp=$_POST['Cuscon'];
	$supcon=$_POST['Cusloc'];
	$suplocation=$_POST['Cusdate'];
	

	$check="SELECT * FROM customers WHERE CustomerName='$supb' AND Phone='$supp'";
	$run=mysqli_query($connect,$check);
	$count=mysqli_num_rows($run);

	if ($count>0) {

		echo"<script>window.alert('Customer Name already exist')</script>";
		  header("Refresh:0");
		
	}
	else{
		$insert="INSERT INTO customers( `CustomerName`, `Phone`, `Address`, `ActiveDate`) VALUES 
		 ('$supb','$supp','$supcon','$suplocation')" ;
...
<li>
<a href="addcustomers.php" class="active">Add Customers</a>
</li>
...
<div class="form-group">
<label>Contact</label>
<input type="text" name="Cuscon" id="phoneNumberInput" onchange="validatePhoneNumber()" required>
</div>
</div>
<div class="col-lg-4 col-sm-6 col-12">
<div class="form-group">
<label>Address</label>
<input type="text" name="Cusloc" required>
</div>
</div>
<div class="col-lg-4 col-sm-6 col-12">
<div class="form-group">
<label>Date</label>
<input type="date" name="Cusdate" required>
</div>
</div>
