...
if(isset($_POST['btnUpd'])) 
{
    ...
    $result=mysqli_query($connect,"SELECT * FROM orderdetails WHERE OrderID='$txtPurchaseID'");
if ($sts=='Approved') {
    ...
    while($arr=mysqli_fetch_array($result))
    {
       ...
       $check="SELECT * FROM inventory WHERE InvID='$ProductID'";
       $run=mysqli_query($connect,$check);
       $arrquantity=mysqli_fetch_array($run);

       if ($arrquantity['Quantity']>=$PurchaseQuantity){
        ...
        $ret=mysqli_query($connect,$UpdateQty);
         ...
         echo "<script>window.alert('This product is out of stock!')</script>";
...
$query1="SELECT p.*,c.*,st.*,r.* FROM orders p,customers c,staffs st,roles r
        WHERE p.OrderID='$purid' AND p.CustomerID=c.CustomerID AND p.StaffID=st.StaffID AND st.RoleID=r.RoleID
        ";
$result1=mysqli_query($connect,$query1);
$row1=mysqli_fetch_array($result1);


$query2="SELECT ord.*,pd.*,inv.*,pro.*
        FROM orders ord,orderdetails pd,inventory inv,products pro
        WHERE ord.OrderID='$purid'
        AND pd.InvID=inv.InvID
        AND inv.ProductID=pro.ProductID
        AND ord.OrderID=pd.OrderID
        ";
...
<font style="vertical-align: inherit;"><font style="vertical-align: inherit;font-size: 14px;color:#000;font-weight: 400;"> <a href="##" class="__cf_email__" data-cfemail="9ffefbf2f6f1dffae7fef2eff3fab1fcf0f2"><?php echo $row1['Phone']; ?></a></font></font><br>
<font style="vertical-align: inherit;"><font style="vertical-align: inherit;font-size: 14px;color:#000;font-weight: 400;"><?php echo $row1['Address']; ?></font></font><br>
<br>
</td>
<td style="padding:5px;vertical-align:top;text-align:left;padding-bottom:20px">
<font style="vertical-align: inherit;margin-bottom:25px;"><font style="vertical-align: inherit;font-size:14px;color:#7367F0;font-weight:600;line-height: 35px; ">Order Info</font></font><br>
...
<font style="vertical-align: inherit;"><font style="vertical-align: inherit;font-size: 14px;color:#2E7D32;font-weight: 400;"> <?php echo $row1['OrderDate'] ?></font></font><br>
<font style="vertical-align: inherit;"><font style="vertical-align: inherit;font-size: 14px;color:#2E7D32;font-weight: 400;"><?php echo $row1['OrderStatus'] ?> </font></font><br>
</td>
</tr>
</tbody></table>
</td>
...
<ul>
<li>
<h4>Total Purchase Price (Include tax)</h4>
<h5><?php echo $row1['TotalAmount'] ?></h5>
</li>
...
<form action="orderconfirm.php" method="POST"> 
    <input type="text" name="updPurid" value="<?php echo $row1['OrderID'] ?>" readonly hidden>
...
<div class="col-lg-3 col-sm-6 col-12">
    <div class="form-group">
        ...
        <select class="select" name="updPurstatus">
            ...
            <?php if ($row1['OrderStatus'] === 'Pending') : ?>
                <option value="Pending" selected>Pending</option>
                <option value="Approved">Approved</option>
                <option value="Canceled">Canceled</option>
            <?php elseif ($row1['OrderStatus'] === 'Canceled') : ?>
...
<div class="col-lg-12 no-print">
    <input type="submit" class="btn btn-submit me-2" name="btnUpd" value="Update" <?php echo ($row1['OrderStatus'] === 'Canceled') ? 'disabled' : ''; ?>>
</div>
