 ...
 */ (function ($, window, document, undefined) {
  ...
  function Plugin(element, options) {
    ...
    this._displacement =
      (-1 * (this.settings.from - this.settings.to)) /
      Math.abs(this.settings.from - this.settings.to);
  ...
  Plugin.prototype = {
    init: function () {
      ...
      if (this.settings.autostart) this._start();
    },
    destroy: function () {
    ...
    _stop: function () {
      this.destroy();
      if (
        this.settings.timerEnd &&
        "function" === typeof this.settings.timerEnd
      ) {
    ...
    _updateTimerText: function () {
      ...
      this.$element.text(function (index, text) {
        ...
        if (_parsedStuff.days < 10) _daysParsed = "0" + _daysParsed;
        if (_parsedStuff.hours < 10) _hoursParsed = "0" + _hoursParsed;
        if (_parsedStuff.minutes < 10) _minutesParsed = "0" + _minutesParsed;
        if (_parsedStuff.seconds < 10) _secondsParsed = "0" + _secondsParsed;
        return that.settings.outputPattern.replace(
          outputReplacement,
