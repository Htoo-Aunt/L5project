...
if (isset($_POST['staffsub'])) {
	...
	$check="SELECT * FROM staffs WHERE Staffname='$staffname'";
	$run=mysqli_query($connect,$check);
	$count=mysqli_num_rows($run);

	if ($count>0) {

		echo"<script>window.alert('Staff Name already exist')</script>";
		  header("Refresh:0");
		
	}
	else{
		$insert=" INSERT INTO staffs(`StaffName`, `UserName`, `Password`, `RegisteredDate`, `RoleID`,`StaffStatus`) VALUES ('$staffname','$stfUname','$stfpass','$stfreg','$stfrole','$stfstatus')";
...
<li>
<a href="addstaff.php" class="active"> Add Staffs</a>
</li>
...
<input type="text" name="stfname" required>
</div>
</div>
...
<input type="text" name="stfusername" required>
</div>
</div>
<div class="col-lg-4 col-sm-6 col-12">
<div class="form-group">
...
<input type="password" class="pass-input" placeholder="Enter your password" name="stfpsw" required>
...
<input type="date" name="stfRegdate" required>
</div>
</div>
...
for ($i=0; $i < $count; $i++) { 
	...
	echo "<option value='$rolID'>$rolName</option>";
} 
...
<div class="form-group">
			...
			<input type="radio" name="stfstatus" value="Active" checked>Active
