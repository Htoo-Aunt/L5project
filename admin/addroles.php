...
if (isset($_POST['rolesub'])) {
	$rolename=$_POST['rolname'];
	
	$rolstatus=$_POST['rolstatus'];

	$check="SELECT * FROM roles WHERE RoleName='$rolename'";
	$run=mysqli_query($connect,$check);
	$count=mysqli_num_rows($run);

	if ($count>0) {

		echo"<script>window.alert('Staff Role already exist')</script>";
		  header("Refresh:0");
		
	}
	else{
		$insert=" INSERT INTO roles(`RoleName`,`RoleStatus`) VALUES ('$rolename','$rolstatus')";
...
<li>
<a href="addroles.php" class="active"> Add roles</a>
</li>
<li>
...
<h4>Add Staff Roles</h4>
<h6>Create new Staff Role</h6>
</div>
</div>
...
<input type="text" name="rolname" id="alphabetInput" onchange="validateAlphabet()" required>
</div>
</div>
...
<textarea class="form-control" name="roldes" required></textarea>
</div>
</div>
<div>
<div class="form-group">
			<label>Active Role?</label>
			<input type="radio" name="rolstatus" value="active" checked>Active
			<input type="radio" name="rolstatus" value="unactive" style="margin-left: 30px;">Unactive

		</div></div>

<div class="col-lg-12">
<input type="submit" class="btn btn-submit me-2" name="rolesub">
...
function validateAlphabet() {
      ...
      // Define a regular expression pattern for alphabets
      const alphabetRegex = /^[a-zA-Z]+$/;

      // Test the input against the regular expression
      const isValid = alphabetRegex.test(userInput);
